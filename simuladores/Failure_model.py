...
class FailureModel(mosaik_api.Simulator):
    def __init__(self):
        super().__init__(META)
        self.line_status = {}  # Diccionario para almacenar el estado de todas las líneas
        self.wind_speed = 0
        self.grid_x = None
        self.grid_y = None
        self.wind_field = None
    ...
    def step(self, time, inputs, max_advance):
        ...
        if self.wind_field is not None and self.line_positions:
            ...
            for lid, lp in self.line_positions.items():
                P, max_v = self.compute_wind_failure(lp['x0'], lp['y0'], lp['x1'], lp['y1'])
                new_line_status[lid] = 0 if np.random.rand() < P else 1
            self.line_status.update(new_line_status)
